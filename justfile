...
setup-gitea:
	...
	echo "  SECRET_URI=\$(juju add-secret git-pat credentials-personal-access-token=${token})"
	echo "  juju deploy git-integrator --channel 1.0/edge \\"
	echo "    --config repository_url=${svc_url} \\"
	echo "    --config tracking_ref=main \\"
	echo "    --config authentication_method=credentials \\"
	echo "    --config credentials_username=${GITEA_USER} \\"
	echo "    --config credentials_personal_access_token_secret=\\\$SECRET_URI"
	echo "  juju grant-secret git-pat git-integrator"
	echo "  juju integrate nifi-k8s:git-registry git-integrator:git"
